...
void SA()
{
    ...
    double ts = 30, te = 1, last_pc = 0, pc, tot = log(ts / te), alpha = 1000;
    for (double tem = ts; tem > te; tem *= 0.99999999)
    {
        ...
        if (pc - last_pc > 0.01)
        {
            printf("%d%%. E change is %lf\n", int(pc * 100), p.e_change);
            last_pc = pc;
            alpha *= 10;
        }
    }
...
int main()
{
    ...
    for (int t = 0; t < 1; t++)
    {
        ...
        length = 0;
        for (int i = 1; i <= sn; i++)
            length += dis(T[p.sq[i - 1]], T[p.sq[i]]);
        if (mn_l > length)
        {
            mn_l = length;
            opt = &p;
